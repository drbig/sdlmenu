...
%w{ sdl thread }.each {|g| require g }

FONT = '/home/pi/sample.ttf'
MENU = [
  ['Game Boy Advance', 'cd /home/pi/gpSP && sudo ./gpsp'],
  ['Super Nintendo', 'cd /home/pi/pisnes && sudo ./snes9x.gui'],
  ['Commodore 64', 'sudo x64'],
  ['Exit', nil],
]
...
class SDLMenu
  ...
  def run!
    ...
    @clock = Thread.new do
      while true
        now = Time.now
        str = now.strftime('%A %Y-%m-%d %H:%M %Z')
